...
import {
  ...
  Play, Pause, Volume2, VolumeX, Maximize2, Clock, Lock, Star, Award, FileText, PlayCircle, Link as LinkIcon,
...
function getYoutubeEmbedUrl(url) {
  ...
  return id ? `https://www.youtube-nocookie.com/embed/${id}?rel=0&modestbranding=1&iv_load_policy=3&controls=0` : null;
}
...
function AppProvider({ children }) {
  const [courses, setCourses] = useState(() => {
    try { const saved = localStorage.getItem('lh_courses'); return saved ? JSON.parse(saved) : INITIAL_COURSES; } catch { return INITIAL_COURSES; }
  });
  ...
  const [commentsMap, setCommentsMap] = useState(() => {
    try {
      const saved = localStorage.getItem('lh_comments');
  ...
  useEffect(() => { localStorage.setItem('lh_courses', JSON.stringify(courses)); }, [courses]);
  ...
  useEffect(() => { localStorage.setItem('lh_comments', JSON.stringify(commentsMap)); }, [commentsMap]);
  useEffect(() => {
...
function YouTubePlayer({ url }) {
  ...
  useEffect(() => {
    ...
    const init = () => {
      playerRef.current = new window.YT.Player(containerRef.current, {
        videoId,
        playerVars: { controls: 0, rel: 0, modestbranding: 1, iv_load_policy: 3, disablekb: 1, fs: 0 },
        events: {
  ...
  const seek = (e) => {
    if (!ready || !progressRef.current) return;
  ...
  return (
    <div className="relative w-full bg-black rounded-2xl overflow-hidden select-none group"
      style={{ paddingTop: '56.25%' }}
      onContextMenu={e => e.preventDefault()}
      onMouseMove={onMouseMove}
      onMouseLeave={() => setShowControls(false)}>
      <div ref={containerRef} className="absolute top-0 left-0 w-full h-full" />
      {/* Zone de clic play/pause */}
      <div className="absolute top-0 left-0 w-full z-10" style={{ height: 'calc(100% - 52px)', cursor: 'pointer' }}
        onClick={togglePlay} onContextMenu={e => e.preventDefault()} />
      {/* Bouton play central (avant démarrage) */}
      {!playing && ready && (
      ...
      <div className={`absolute bottom-0 left-0 w-full z-30 transition-opacity duration-300 ${showControls || !playing ? 'opacity-100' : 'opacity-0'}`}
        style={{ background: 'linear-gradient(transparent, rgba(0,0,0,0.85))', padding: '20px 12px 10px' }}>
        {/* Barre de progression */}
        <div ref={progressRef} className="w-full h-1 bg-white/30 rounded-full mb-2 cursor-pointer relative"
          onClick={seek}>
